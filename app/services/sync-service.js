...
exports.startUserSync = function (callback) {
    ...
    async.series([
        ...
        function (callback) {
            Users.find(/*{updateCount:{$ne:DSCount},smartCardNumber:{$ne:''},*/{$where:"this.lastModifiedAt>this.lastSyncedAt"},function (err,result) {
                if(err)
                {
                    return callback(err,null);
                }
        ...
        function (callback) {
            if(allUsers.length>0)
            {
                for(var i=0;i<allUsers.length;i++)
                {
                    ...
                    UsersDict.set(user.UserID,user);
...
/*                                success.push(obj.unsuccessIp[j]);
                                ...
                                Users.findByOneAndUpdate({UserID:result.UserID},result,function (err,result) {
                                    if(err)
                                    {
                                        console.error('Error in Updating user after getting response');
                                    }
